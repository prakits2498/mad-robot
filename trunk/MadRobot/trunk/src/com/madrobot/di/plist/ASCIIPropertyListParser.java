...
public class ASCIIPropertyListParser {
	...
	private final static Pattern scannerDelimiterPattern = Pattern
	...
	private final static Pattern simpleStringPattern = Pattern
			.compile("[\\x00-\\x7F&&[^\" ,;\\(\\)\\{\\}\\<\\>]]+");
	...
	private final static Pattern gnuStepDateBeginPattern = Pattern
			.compile("<\\*D[0-9]{4}-[0-1][0-9]-[0-3][0-9]");
	// yyyy-MM-ddTHH:mm:ssZ
	private final static Pattern appleDatePattern = Pattern
			.compile("\"[0-9]{4}-[0-1][0-9]-[0-3][0-9]T[0-2][0-9]:[0-5][0-9]:[0-5][0-9]Z\"");

	private final static Pattern appleBooleanPattern = Pattern.compile("(YES)|(NO)");
	private final static Pattern gnuStepBooleanPattern = Pattern
			.compile("(\\<\\*BY\\>)|(\\<\\*BN\\>)");

	private final static Pattern gnuStepIntPattern = Pattern.compile("\\<\\*I[0-9]+\\>");
	private final static Pattern gnuStepRealPattern = Pattern
			.compile("\\<\\*R[0-9]+(.[0-9]+)?\\>");
	...
	private static NSObject parse(Scanner s) throws Exception {
		...
		if (s.hasNext(arrayBeginToken) || s.hasNext(dictionaryBeginToken)) {
			...
			throw new ParseException("Expected '" + arrayBeginToken + "' or '"
					+ dictionaryBeginToken + "' but found " + s.next(), 0);
		}
	}
	...
	private static NSObject parseObject(Scanner s) throws Exception {
		if (s.hasNext(arrayBeginToken)) {
			...
			if (!s.hasNext(arrayEndToken)) {
				throw new ParseException("Expected '" + arrayEndToken + "' but found "
						+ s.next(), 0);
			}
			s.next();
			...
			if (!s.hasNext(dictionaryEndToken)) {
				throw new ParseException("Expected '" + dictionaryEndToken + "' but found "
						+ s.next(), 0);
			}
			s.next();
			return dict;
	...
	private static String parseEscapedSequence(StringCharacterIterator iterator)
		...
		if (c == '\\') {
			...
			byte2 += iterator.next();
			byte2 += iterator.next();
			byte[] stringBytes = { (byte) Integer.parseInt(byte1, 16),
					(byte) Integer.parseInt(byte2, 16) };
