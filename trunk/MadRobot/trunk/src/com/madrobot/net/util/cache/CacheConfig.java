...
/**
 * <p>
 * Java Beans-style configuration for a {@link CachingHttpClient}. Any class in
 * the caching module that has configuration options should take a
 ...
 * easiest way to specify options is to get an instance and then set just the
 * options you want to modify from their defaults.
 * </p>
 * 
 * <p>
 * <b>N.B.</b> This class is only for caching-specific configuration; to
 * configure the behavior of the rest of the client, configure the
 * {@link org.apache.http.client.HttpClient} used as the &quot;backend&quot; for
 * the {@code CachingHttpClient}.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * <b>Cache size.</b> If the backend storage supports these limits, you can
 * specify the {@link CacheConfig#setMaxCacheEntries maximum number of cache
 * entries} as well as the {@link CacheConfig#setMaxObjectSizeBytes maximum
 * cacheable response body size}.
 * </p>
 * 
 * <p>
 * <b>Public/private caching.</b> By default, the caching module considers
 * itself to be a shared (public) cache, and will not, for example, cache
 * responses to requests with {@code Authorization} headers or responses marked
 * with {@code Cache-Control: private}. If, however, the cache is only going to
 * be used by one logical "user" (behaving similarly to a browser cache), then
 * you will want to {@link CacheConfig#setSharedCache(boolean) turn off the
 * shared cache setting}.
 * </p>
 * 
 * <p>
 * <b>Heuristic caching</b>. Per RFC2616, a cache may cache certain cache
 * entries even if no explicit cache control headers are set by the origin. This
 * behavior is off by default, but you may want to turn this on if you are
 * working with an origin that doesn't set proper headers but where you still
 * want to cache the responses. You will want to
 * {@link CacheConfig#setHeuristicCachingEnabled(boolean) enable heuristic
 * caching}, then specify either a
 * {@link CacheConfig#setHeuristicDefaultLifetime(long) default freshness
 * lifetime} and/or a {@link CacheConfig#setHeuristicCoefficient(float) fraction
 * of the time since the resource was last modified}. See Sections <a
 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html#sec13.2.2">
 * 13.2.2</a> and <a
 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html#sec13.2.4">
 * 13.2.4</a> of the HTTP/1.1 RFC for more details on heuristic caching.
 * </p>
 * 
 * <p>
 * <b>Background validation</b>. The cache module supports the
 * {@code stale-while-revalidate} directive of <a
 * href="http://tools.ietf.org/html/rfc5861">RFC5861</a>, which allows certain
 * cache entry revalidations to happen in the background. You may want to tweak
 * the settings for the {@link CacheConfig#setAsynchronousWorkersCore(int)
 * minimum} and {@link CacheConfig#setAsynchronousWorkersMax(int) maximum}
 * number of background worker threads, as well as the
 * {@link CacheConfig#setAsynchronousWorkerIdleLifetimeSecs(int) maximum time
 * they can be idle before being reclaimed}. You can also control the
 * {@link CacheConfig#setRevalidationQueueSize(int) size of the queue} used for
...
public class CacheConfig {
	 ...
	 * from the Last-Modified time of a cache entry.
	 */
	...
	/**
	 * Default maximum idle lifetime for a background revalidation thread before
	 * it gets reclaimed.
	 */
	...
	/**
	 * Specifies the maximum response body size that will be eligible for
	 * caching.
	 * 
	 ...
	 * 
	 * @return {@code true} for a shared cache, {@code false} for a non- shared
	 *         (private) cache
	 */
	 ...
	 *            non-shared (private) cache. To have the cache behave like a
	 *            browser cache, you want to set this to {@code false}.
	 */
	 ...
	 * 
	 * @param heuristicCachingEnabled
	 *            should be {@code true} to permit heuristic caching,
	 ...
	 *            should be between {@code 0.0} and {@code 1.0}.
	 */
	 ...
	 * calculation is not possible. Explicit cache control directives on either
	 * the request or origin response will override this, as will the heuristic
	 * {@code Last-Modified} freshness calculation if it is available.
	 * 
	 * @param heuristicDefaultLifetimeSecs
	 *            is the number of seconds to consider a cache-eligible response
	 *            fresh in the absence of other information. Set this to
	 *            {@code 0} to disable this style of heuristic caching.
	 */
	...
	/**
	 * Sets the maximum number of threads to allow for background revalidations
	 * due to the {@code stale-while-revalidate} directive.
	 * 
	 * @param max
	 ...
	 * 
	 * @param min
	 *            should be greater than zero and less than or equal to
	 *            <code>getAsynchronousWorkersMax()</code>
	 */
	...
	/**
	 * Returns the current maximum idle lifetime in seconds for a background
	 * revalidation worker thread. If a worker thread is idle for this long, and
	 * there are more than the core number of worker threads alive, the worker
	 * will be reclaimed.
	 */
	...
	/**
	 * Sets the current maximum idle lifetime in seconds for a background
	 * revalidation worker thread. If a worker thread is idle for this long, and
	 * there are more than the core number of worker threads alive, the worker
	 * will be reclaimed.
	 * 
