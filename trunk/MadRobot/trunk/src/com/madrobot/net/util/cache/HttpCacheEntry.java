...
public class HttpCacheEntry implements Serializable {
	 ...
	 * @param variantMap
	 *            describing cache entries that are variants of this parent
	 *            entry; this maps a "variant key" (derived from the varying
	 *            request headers) to a "cache key" (where in the cache storage
	 *            the particular variant is located)
	 */
	public HttpCacheEntry(final Date requestDate, final Date responseDate,
			...
			final Resource resource, final Map<String, String> variantMap) {
		super();
		if (requestDate == null) {
	...
	/**
	 * Indicates whether the origin response indicated the associated resource
	 * had variants (i.e. that the Vary header was set on the origin response).
	 * 
	 ...
	 * resource are stored. This maps "variant keys" to "cache keys", where the
	 * variant key is derived from the varying request headers, and the cache
	 * key is the location in the
	 * {@link com.madrobot.net.util.cache.HttpCacheStorage} where that
	 * particular variant is stored. The first variant returned is used as the
	 * "parent" entry to hold this index of the other variants.
	 */
	...
	/**
	 * Provides a string representation of this instance suitable for human
	 * consumption.
	 */
	@Override
