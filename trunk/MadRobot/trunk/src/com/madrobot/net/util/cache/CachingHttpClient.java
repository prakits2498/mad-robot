...
/**
 * <p>
 * The {@link CachingHttpClient} is meant to be a drop-in replacement for a
 * {@link DefaultHttpClient} that transparently adds client-side caching. The
 * current implementation is conditionally compliant with HTTP/1.1 (meaning all
 * the MUST and MUST NOTs are obeyed), although quite a lot, though not all, of
 * the SHOULDs and SHOULD NOTs are obeyed too. Generally speaking, you construct
 * a {@code CachingHttpClient} by providing a "backend" {@link HttpClient} used
 * for making actual network requests and provide an {@link HttpCacheStorage}
 * instance to use for holding onto cached responses. Additional configuration
 * options can be provided by passing in a {@link CacheConfig}. Note that all of
 * the usual client related configuration you want to do vis-a-vis timeouts and
 * connection pools should be done on this backend client before constructing a
 * {@code CachingHttpClient} from it.
 * </p>
 * 
 * <p>
 * Generally speaking, the {@code CachingHttpClient} is implemented as a <a
 * href="http://en.wikipedia.org/wiki/Decorator_pattern">Decorator</a> of the
 * backend client; for any incoming request it attempts to satisfy it from the
 * cache, but if it can't, or if it needs to revalidate a stale cache entry, it
 * will use the backend client to make an actual request. However, a proper
 * HTTP/1.1 cache won't change the semantics of a request and response; in
 * particular, if you issue an unconditional request you will get a full
 * response (although it may be served to you from the cache, or the cache may
 * make a conditional request on your behalf to the origin). This notion of
 * "semantic transparency" means you should be able to drop a
 * {@link CachingHttpClient} into an existing application without breaking
 * anything.
 * </p>
 * 
 * <p>
 * Folks that would like to experiment with alternative storage backends should
 * look at the {@link HttpCacheStorage} interface and the related package
 * documentation there. You may also be interested in the provided
...
public class CachingHttpClient implements HttpClient {
	 ...
	 * {@link com.madrobot.net.util.cache.CacheResponseStatus} of a request (for
	 * example, whether it resulted in a cache hit) will be recorded if an
	...
	private final Map<ProtocolVersion, String> viaHeaders = new HashMap<ProtocolVersion, String>(
			4);
	 ...
	 * that stores cache entries in memory and uses a vanilla
	 * {@link DefaultHttpClient} for backend requests.
	 * 
	 * @param config
	...
	/**
	 * Constructs a {@code CachingHttpClient} with the given caching options
	 * that stores cache entries in memory and uses the given {@link HttpClient}
	...
	/**
	 * Constructs a {@code CachingHttpClient} with the given caching options
	 * that stores cache entries in the provided storage backend and uses the
	 * given {@link HttpClient} for backend requests. However, cached response
	 * bodies are managed using the given {@link ResourceFactory}.
	 * 
	 * @param client
	 *            used to make origin requests
	 * @param resourceFactory
	 *            how to manage cached response bodies
	 * @param storage
	 *            where to store cache entries
	 * @param config
	 *            cache module options
	 */
	public CachingHttpClient(HttpClient client, ResourceFactory resourceFactory,
	 ...
	 * Constructs a {@code CachingHttpClient} with the given caching options
	 * that stores cache entries in the provided storage backend and uses the
	 * given {@link HttpClient} for backend requests.
	 * 
	 * @param client
	 *            used to make origin requests
	 * @param storage
	 ...
	 * {@link HttpRequest} without contacting the origin server.
	 * 
	...
	/**
	 * Reports the number of times that the cache contacted the origin server
	 * because it had no appropriate response cached.
	 * 
	...
	/**
	 * Reports the number of times that the cache was able to satisfy a response
	 * by revalidating an existing but stale cache entry.
	 * 
	...
	private <T> T handleAndConsume(final ResponseHandler<? extends T> responseHandler,
		...
		try {
			...
			try {
				...
				Log.d("MadRobot", "Error consuming content after an exception.", t2);
			}
	...
	private HttpResponse handleCacheHit(HttpHost target, HttpRequest request,
			HttpContext context, HttpCacheEntry entry) throws ClientProtocolException,
			IOException {
	...
	private HttpResponse revalidateCacheEntry(HttpHost target, HttpRequest request,
			HttpContext context, HttpCacheEntry entry, Date now)
			throws ClientProtocolException {
		Log.d("MadRobot", "Revalidating the cache entry");
	...
	private HttpCacheEntry satisfyFromCache(HttpHost target, HttpRequest request) {
		...
		try {
			...
			Log.d("MadRobot", "Unable to retrieve entries from cache", ioe);
		}
		return entry;
	}
	...
	private Map<String, Variant> getExistingCacheVariants(HttpHost target, HttpRequest request) {
		...
		try {
			...
			Log.d("MadRobot", "Unable to retrieve variant entries from cache", ioe);
		}
		return variants;
	}
	...
	private void flushEntriesInvalidatedByRequest(HttpHost target, HttpRequest request) {
		try {
			...
			Log.d("MadRobot", "Unable to flush invalidated entries from cache", ioe);
		}
	}

	private HttpResponse generateCachedResponse(HttpRequest request, HttpContext context,
		...
		if (validityPolicy.getStalenessSecs(entry, now) > 0L) {
			cachedResponse.addHeader(HttpConstants.WARNING,
					"110 localhost \"Response is stale\"");
		}
		return cachedResponse;
	}

	private HttpResponse handleRevalidationFailure(HttpRequest request, HttpContext context,
			HttpCacheEntry entry, Date now) {
	...
	private String generateViaHeader(HttpMessage msg) {
		...
		if ("http".equalsIgnoreCase(pv.getProtocol())) {
			value = String.format("%d.%d localhost (Apache-HttpClient/%s (cache))",
					pv.getMajor(), pv.getMinor(), release);
		} else {
			value = String.format("%s/%d.%d localhost (Apache-HttpClient/%s (cache))",
	...
	/**
	 * Reports whether this {@code CachingHttpClient} implementation supports
	 * byte-range requests as specified by the {@code Range} and
	 * {@code Content-Range} headers.
	 * 
	...
	/**
	 * Reports whether this {@code CachingHttpClient} is configured as a shared
	 * (public) or non-shared (private) cache. See
	 * {@link CacheConfig#setSharedCache(boolean)}.
	 * 
	 * @return {@code true} if we are behaving as a shared (public) cache
	 */
	...
	boolean clientRequestsOurOptions(HttpRequest request) {
		...
		if (!HttpMethod.OPTIONS.equals(line.getMethod()))
			return false;
	...
	HttpResponse callBackend(HttpHost target, HttpRequest request, HttpContext context)
		...
		Log.d("MadRobot", "Calling the backend");
	...
	HttpResponse negotiateResponseFromVariants(HttpHost target, HttpRequest request,
			HttpContext context, Map<String, Variant> variants) throws IOException {
		...
		if (backendResponse.getStatusLine().getStatusCode() != HttpStatus.SC_NOT_MODIFIED) {
			return handleBackendResponse(target, request, requestDate, responseDate,
					backendResponse);
		}
		...
		if (matchingVariant == null) {
			Log.d("MadRobot",
					"304 response did not contain ETag matching one sent in If-None-Match");
			return callBackend(target, request, context);
		}
		...
		HttpCacheEntry responseEntry = getUpdatedVariantEntry(target, conditionalRequest,
	...
	private HttpResponse retryRequestUnconditionally(HttpHost target, HttpRequest request,
			HttpContext context, HttpCacheEntry matchedEntry) throws IOException {
	...
	private HttpCacheEntry getUpdatedVariantEntry(HttpHost target,
			HttpRequest conditionalRequest, Date requestDate, Date responseDate,
			HttpResponse backendResponse, Variant matchingVariant, HttpCacheEntry matchedEntry) {
		HttpCacheEntry responseEntry = matchedEntry;
		try {
			responseEntry = responseCache.updateVariantCacheEntry(target, conditionalRequest,
					matchedEntry, backendResponse, requestDate, responseDate,
					matchingVariant.getCacheKey());
		} catch (IOException ioe) {
			Log.d("MadRobot", "Could not update cache entry", ioe);
		}
	...
	private void tryToUpdateVariantMap(HttpHost target, HttpRequest request,
		...
		try {
			...
			Log.d("MadRobot", "Could not update cache entry to reuse variant", ioe);
		}
	}

	private boolean shouldSendNotModifiedResponse(HttpRequest request,
			HttpCacheEntry responseEntry) {
		return (suitabilityChecker.isConditional(request) && suitabilityChecker
				.allConditionalsMatch(request, responseEntry, new Date()));
	}
	...
	HttpResponse revalidateCacheEntry(HttpHost target, HttpRequest request,
		...
		HttpRequest conditionalRequest = conditionalRequestBuilder.buildConditionalRequest(
				request, cacheEntry);
		...
		if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
			HttpCacheEntry updatedEntry = responseCache.updateCacheEntry(target, request,
	...
	HttpResponse handleBackendResponse(HttpHost target, HttpRequest request, Date requestDate,
			Date responseDate, HttpResponse backendResponse) throws IOException {

		Log.d("MadRobot", "Handling Backend response");
		responseCompliance.ensureProtocolCompliance(request, backendResponse);

		boolean cacheable = responseCachingPolicy
				.isResponseCacheable(request, backendResponse);
		...
		if (cacheable && !alreadyHaveNewerCacheEntry(target, request, backendResponse)) {
			try {
				...
				Log.d("MadRobot", "Unable to store entries in cache", ioe);
			}
		}
		if (!cacheable) {
			try {
				...
				Log.d("MadRobot", "Unable to flush invalid cache entries", ioe);
			}
		}
