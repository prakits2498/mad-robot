...
/**
 * Implementation of the so-called draft76 WebSocket specification. This version
 * of the spec is officially expired but widely implemented.
 * <p>
	...
	public Message readMessage(WebSocket socket, DataInputStream input)
		...
		for (;;) {
			...
			if ((frameType & 0x80) == 0x80) {
				int length = 0;
				for (;;) {
					int b = input.readUnsignedByte(), bv = b & 0x7f;
				...
				if (frameType == 0xff && length == 0) {
					...
					if (readyState == WebSocket.OPEN) {
						...
						// Block on reading the next byte which should just be a
						// stream closed
