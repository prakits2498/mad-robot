...
public class WireProtocol {
	...
	public void performHandshake(WebSocket socket, URI uri, DataInputStream in,
		...
		for (;;) {
			...
			String name = headerLine.substring(0, colonPos), value = headerLine
	 ...
	 * 
	 * @param charCount
	 *            Number of misc ascii chars
	 * @param numCount
	 *            Number of numeric chars
	 * @param spaceCount
	 *            Number of spaces
	 * @param armor
	 *            true to armor the result (add a printable ascii char to front
	 *            and back)
	 * @return the result
	 */
	public static String generateKey() {
		int divisor = random.nextInt(8) + 1;
		int number = (random.nextInt(1000000) + 1000) * divisor; // Make sure it
																	// evenly
																	// divides
	...
	protected String readLine(InputStream in) throws IOException {
		...
		CharsetDecoder decoder = StringUtils.UTF8.newDecoder();
		...
		for (;;) {
			...
			for (lineEndIndex = 0; lineEndIndex < r; lineEndIndex++) {
				...
				if (b == '\n') {
					...
					// Treat this as an end of line but set a flag saying we
