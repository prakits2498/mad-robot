 ...
 * {@linkplain "http://code.google.com/p/gears/wiki/ResumableHttpRequestsProposal"}
 * ). This implementation supports time based progress notifications, polling
 * for progress, resumability, and completion notifications.
 * 
 * Each instance creates a separate task (to be executed by an ExecutorService),
 * which actually generates the HTTP request and writes bytes to the server. The
 * task blocks a thread spawned by the ExecutorService) for the duration of the
 * upload (i.e., until the upload is either completed, paused, or an error
...
public class ResumableHttpFileUploader {
	...
	public static class ResponseMessage {
		...
		/**
		 * Returns the last request's connection's input stream to read the
		 * response body from.
		 * 
		 ...
		 *             if a {@link IOException} is thrown while reading from the
		 *             input stream.
...
//	}
	...
	private class NotificationTask extends TimerTask {
		...
		private final IOProgressCallback listener;
		private final Timer timer;

		public NotificationTask(ResumableHttpFileUploader fileUploader,
				IOProgressCallback listener, Timer timer) {
		...
		@Override
		public void run() {
			if (!fileUploader.getUploadState().equals(IOState.IN_PROGRESS)) {
				timer.cancel();
			}
			listener.progressPercent(fileUploader.getProgress());
		}
	}
	 ...
	 */
	private IOState uploadState = IOState.NOT_STARTED;

	/**
	 * The future which will contain the eventual response stream from the
	 * upload server.
	 */
	 ...
	 */
	private final IOProgressCallback progressListener;
	...
	/**
	 * Back off policy which determines the amount of time to wait before
	...
	/**
	 * Builder class for constructing {@link ResumableHttpFileUploader}
	 * instances.
	 */
	public static class Builder {
		 ...
		 * @return this
		 */
		public Builder setProgressListener(IOProgressCallback progressListener) {
		...
		/**
		 * @param progressIntervalMillis
		 *            number of milliseconds between progress listener
		 *            notifications
		 * @return this
		 */
		 ...
		 * HTTP settings
		 * <p>
		 * The http method can be POST or PUT.
		 * </p>
		 * 
		 * @param httpSettings
		 *            the http request type for upload. Use either PUT request
		 *            or POST request with x-http-method-override header set to
		 *            PUT.
		 * @return this
		 */
		...
		/**
		 * @param backoffPolicy
		 *            to determine how long to wait until retrying HTTP requests
		 * @return this
		 */
	 ...
	 *            which locates the destination of the upload request
	 * @param file
	 *            containing bytes to send to the server
	 * @param executor
	 *            service to execute asynchronous upload tasks with
	 * @param progressListener
	 *            for receiving progress notifications
	 ...
	 *            for receiving progress notifications
	 * @param chunkSize
	 *            size of the chunks that will get uploaded by individual HTTP
	 *            requests
	 * @param progressIntervalMillis
	 *            number of milliseconds between progress listener notifications
	 * @throws IOException
	 ...
	 * 
	 * @param builder
	 *            to use to construct this uploader
	 * @throws IOException
	...
	ResumableHttpFileUploader(Builder builder) throws IOException {
		...
		if (HttpMethod.POST.equals(httpRequestMethod.getHttpMethod())) {
			addHeader(METHOD_OVERRIDE, HttpMethod.PUT.toString());
		}
	}
	...
	public double getProgress() {
		...
		if (fileLength == 0) {
			return uploadState.equals(IOState.COMPLETE) ? 1 : 0;
		} else {
	...
	/**
	 * Gets the response from the server if it is available. If the stream is
	 * not yet available, <code>null</code> is returned.
	 * 
	 ...
	 */
	public synchronized IOState getUploadState() {
		return uploadState;
	}
	...
	public synchronized boolean isPaused() {
		return uploadState.equals(IOState.PAUSED);
	}
	...
	/**
	 * Resumes an upload if it is currently paused, or if it has not yet
	 * started. This should be called if the server has received some bytes for
	 * the file. Note that it causes an extra HTTP request to be sent to the
	 * server in order to determine the offset at which the uploader should
	 ...
	 * 
	 * @param numBytes
	 *            to add to the uploaded byte count
	 */
	 ...
	 * 
	 * @param resume
	 *            <code>true</code> if the file should be resumed
	 */
