...
class Card {
	...
	void setCardVertices(float[] cardVertices) {
	...
	void setOrientation(boolean orientationVertical) {
	...
	void draw(GL10 gl) {
		...
		if (angle > 0) {
			...
			gl.glVertexPointer(3, GL_FLOAT, 0, NIOUtils.toFloatBuffer(shadowVertices));
			gl.glDrawElements(GL_TRIANGLES, indices.length, GL_UNSIGNED_SHORT, indexBuffer);
	...
	private void updateVertices() {
		vertexBuffer = NIOUtils.toFloatBuffer(cardVertices);
		indexBuffer = NIOUtils.toShortBuffer(indices);
		textureBuffer = NIOUtils.toFloatBuffer(textureCoordinates);
	}
}