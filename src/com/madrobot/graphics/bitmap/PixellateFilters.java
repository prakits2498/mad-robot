...
public class PixellateFilters {
	...
	public static final Bitmap crystallize(Bitmap bitmap, int gridType, boolean fadeEdges, float angle, float scale, float stretch, float randomNess, float edgeThickness, int edgeColor, Bitmap.Config outputConfig) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				float f = evaluate(nx, ny, results, random, gridType, probabilities, randomNess);
	...
	private static float evaluate(float x, float y, Point[] results, Random random, int gridType, byte[] probabilities, float randomNess) {
		for (int j = 0; j < results.length; j++)
		...
		float d = checkCube(fx, fy, ix, iy, results, random, gridType, probabilities, randomNess);
		if (d > fy)
			d = checkCube(fx, fy + 1, ix, iy - 1, results, random, gridType, probabilities, randomNess);
		if (d > 1 - fy)
			d = checkCube(fx, fy - 1, ix, iy + 1, results, random, gridType, probabilities, randomNess);
		if (d > fx) {
			checkCube(fx + 1, fy, ix - 1, iy, results, random, gridType, probabilities, randomNess);
			if (d > fy)
				d = checkCube(fx + 1, fy + 1, ix - 1, iy - 1, results, random, gridType, probabilities, randomNess);
			if (d > 1 - fy)
				d = checkCube(fx + 1, fy - 1, ix - 1, iy + 1, results, random, gridType, probabilities, randomNess);
		}
		if (d > 1 - fx) {
			d = checkCube(fx - 1, fy, ix + 1, iy, results, random, gridType, probabilities, randomNess);
			if (d > fy)
				d = checkCube(fx - 1, fy + 1, ix + 1, iy - 1, results, random, gridType, probabilities, randomNess);
			if (d > 1 - fy)
				d = checkCube(fx - 1, fy - 1, ix + 1, iy + 1, results, random, gridType, probabilities, randomNess);
	...
	private static float checkCube(float x, float y, int cubeX, int cubeY, Point[] results, Random random, int gridType, byte[] probabilities, float randomness) {
		int numPoints;
