...
public interface OAuthProvider extends Serializable {
	 ...
	 *            1.0 and your app cannot receive callbacks. Please note that
	 *            some services (among them Twitter) will fail authorization if
	 *            you pass a callback URL but register your application as a
	 *            desktop app (which would only be able to handle OOB requests).
	 ...
	 * 
	 * @param parameters
	 *            the map of query parameters served by the service provider in
	 *            the token response
	 */
