...
/**
 * Implement WebSocket protocol draft 03 (numbering reset due to change in
 * working group - draft03 is newer than draft76).
 * <p>
...
public class WireProtocolDraft03 extends WireProtocol {
	...
	public void initiateClose(WebSocket socket) {
		synchronized (socket) {
			byte[] cookie = StringUtils.getUTF8Bytes("clientclose");
		...
		for (;;) {
			...
			if (length == 126) {
				...
				if (longLength > Integer.MAX_VALUE)
					throw new IOException("Message length too long");
			...
			case Message.OPCODE_CLOSE:
				...
				if (closeCookie == null
					...
					// The writer will close the queue when it is done. Just
					// wait for it.
