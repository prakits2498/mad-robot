...
class FileCacheRequest extends CacheRequest {
	...
	private static class TempFileOutputStream extends FilterOutputStream {
		 ...
		 * 
		 * @param out
		 *            the {@link FileOutputStream} to decorate.
		 * @param temp
		 *            the temporary file.
		 * @param file
		 *            the destination file.
		 */
		public TempFileOutputStream(OutputStream out, FileDescriptor fd,
				File temp, File file) {
			super(out);
			if (fd == null) {
	...
	public FileCacheRequest(File file, int responseCode,
			String responseMessage, Map<String, List<String>> headers) {
		if (file == null) {
