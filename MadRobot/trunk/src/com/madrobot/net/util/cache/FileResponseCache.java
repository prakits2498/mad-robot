...
public abstract class FileResponseCache extends ResponseCache {
	...
	public static ContentHandler capture(ContentHandler handler, Object cookie) {
		...
		if (responseCache instanceof FileResponseCache) {
			...
			return new FileResponseCacheContentHandler(handler,
					fileResponseCache, cookie);
		} else if (responseCache == null) {
			throw new IllegalStateException("ResponseCache not found");
		} else {
			Class<? extends ResponseCache> type = responseCache.getClass();
			String message = "Installed ResponseCache is not a FileResponseCache: "
					+ type;
	 ...
	 * 
	 * @param file
	 *            the file in which to save the cached response.
	 * @param connection
	 *            the {@link URLConnection} to be cached.
	 * @return the {@link CacheRequest}.
	 * @throws IOException
	 *             if there is a problem reading the {@link URLConnection}.
	 */
	private CacheRequest createCacheRequest(File file, URLConnection connection)
		...
		if (connection instanceof HttpURLConnection) {
			...
			Log.w(TAG, "URLConnection is not an HttpURLConnection: "
					+ className);
		}
		Map<String, List<String>> responseHeaders = connection
				.getHeaderFields();
		return new FileCacheRequest(file, responseCode, responseMessage,
				responseHeaders);
	}
	 ...
	 * 
	 * @param file
	 *            the file containing the cached response.
	...
	private File getFile(Frame frame) {
		try {
			...
			Map<String, List<String>> requestHeaders = new RequestPropertiesMap(
					http);
	...
	/**
	 * Returns the {@link File} used for caching the given request, or
	 * {@code null} if this type of request should not be cached.
	 * <p>
	 * It is not necessary to create the parent directories; this will be done
	 * automatically before the file is written.
	 * 
	 * @param uri
	 *            the {@link URI} of the request.
	 * @param requestMethod
	 ...
	 *            none was passed.
	 * @return the {@link File} in which to cache the given request, or
	 *         {@code null} to not cache this request.
	 */
	 ...
	 *            the HTTP request method.
	 * @param requestHeaders
	 *            the HTTP request headers. Keys should be specified in
	 *            lower-case.
			...
			for (String value : values) {
				if (value.startsWith(MAX_AGE_PREFIX)) {
					try {
						...
						if (Log.isLoggable(TAG, Log.ERROR)) {
							Log.e(TAG, "Failed to parse Cache-Control: "
									+ value, e);
						}
					}
				}
