...
/**
 * Converts a java.util.TreeMap to XML, and serializes the associated
 * java.util.Comparator. The converter assumes that the entries in the XML are
...
public class TreeMapConverter extends MapConverter {
		...
		try {
			...
			if (cmpField == null) {
				throw new ExceptionInInitializerError(
						"Cannot detect comparator field of TreeMap");
			}
	...
	protected void populateTreeMap(HierarchicalStreamReader reader,
			UnmarshallingContext context, TreeMap result, Comparator comparator) {
		boolean inFirstElement = comparator == NULL_MARKER;
		if (inFirstElement) {
			comparator = null;
		}
		SortedMap sortedMap = new PresortedMap(comparator != null
		...
		try {
			if (JVM.hasOptimizedTreeMapPutAll()) {
				...
				result.putAll(sortedMap); // internal optimization will not call
											// comparator
				...
				result.putAll(sortedMap); // will use comparator for already
											// sorted map
			}
