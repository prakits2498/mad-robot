...
/**
 * Self-contained XStream generator. The class is a utility to write XML streams
 * that contain additionally the XMLWizarda that was used to serialize the
 * object graph. Such a stream can be unmarshalled using this embedded XStream
...
class XMLStreamer {
	 ...
	 * @throws com.madrobot.di.xml.XMLWizardException
	...
	/**
	 * Deserialize a self-contained XStream with object from an XML Reader. The
	 * method will use internally an XppDriver to load the contained XStream
	 * instance.
	 * 
	 * @throws IOException
	 *             if an error occurs reading from the Reader.
	...
	/**
	 * Deserialize a self-contained XStream with object from a String. The
	 * method will use internally an XppDriver to load the contained XStream
	 * instance.
	 * 
	 * @throws ClassNotFoundException
	...
	/**
	 * Serialize an object including the XStream to the given Writer as
	 * pretty-printed XML.
	 * <p>
	 * Warning: XStream will serialize itself into this XML stream. To read such
	 * an XML code, you should use {@link XMLStreamer#fromXML(Reader)} or one of
	 * the other overloaded methods. Since a lot of internals are written into
	 * the stream, you cannot expect to use such an XML to work with another
	 * XStream version or with XStream running on different JDKs and/or
	 * versions. We have currently no JDK 1.3 support, nor will the
