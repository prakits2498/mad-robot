...
/**
 * Converts a {@link Subject} instance. Note, that this Converter does only
 * convert the contained Principals as it is done by JDK serialization, but not
 * any credentials. For other behaviour you can derive your own converter,
 * overload the appropriate methods and register it in the
 * {@link com.madrobot.di.wizard.xml.XMLWizard}.
 * 
 * @since 1.1.3
 */
public class SubjectConverter extends AbstractCollectionConverter {
	...
	protected void marshalPrincipals(Set principals,
		...
		for (final Iterator iter = principals.iterator(); iter.hasNext();) {
			final Object principal = iter.next(); // pre jdk 1.4 a Principal was
													// also in javax.security
			writeItem(principal, context, writer);
		}
		writer.endNode();
	};

	protected void marshalPrivateCredentials(Set privCredentials,
			HierarchicalStreamWriter writer, MarshallingContext context) {
	};

	protected void marshalPublicCredentials(Set pubCredentials,
			HierarchicalStreamWriter writer, MarshallingContext context) {
	};
	...
	protected Set unmarshalPrivateCredentials(HierarchicalStreamReader reader,
			UnmarshallingContext context) {
		return Collections.EMPTY_SET;
	};

	protected Set unmarshalPublicCredentials(HierarchicalStreamReader reader,
			UnmarshallingContext context) {
		return Collections.EMPTY_SET;
	};
