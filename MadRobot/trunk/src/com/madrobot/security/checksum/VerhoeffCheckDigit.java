 ...
 * <p>
 * Check digit calculation for numeric codes using a <a
 * href="http://en.wikipedia.org/wiki/Dihedral_group">Dihedral Group</a> of
 * order 10.
 * <p>
...
public final class VerhoeffCheckDigit implements CheckDigit, Serializable {
	...
	public String calculate(String code) throws ChecksumException {
		if (code == null || code.length() == 0) {
			throw new ChecksumException("Code is missing");
		}
	...
	private int calculateChecksum(String code, boolean includesCheckDigit)
		...
		for (int i = 0; i < code.length(); i++) {
			...
			if (num < 0 || num > 9) {
				throw new ChecksumException("Invalid Character[" + i + "] = '"
						+ ((int) code.charAt(idx)) + "'");
			}
