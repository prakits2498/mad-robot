 ...
 * with the JDK's {@link WeakHashMap WeakHashMap}, which uses weak references
...
public class SoftHashMap<K, V> implements Map<K, V> {
	 ...
	 * 
	 * @param retentionSize
	 *            the total number of most recent entries in the map that will
	 *            be strongly referenced (retained), preventing them from being
	 *            eagerly garbage collected by the JVM.
	 */
	...
	private static class SoftValue<V, K> extends SoftReference<V> {
		 ...
		 *            the map key
		 * @param queue
		 *            the soft reference queue to poll to determine if the entry
		 *            had been reaped by the GC.
		 */
